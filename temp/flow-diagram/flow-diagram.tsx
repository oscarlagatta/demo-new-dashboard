...
import SplunkAgGrid from "./splunk-ag-grid"
...
const Flow = () => {
  ...
  // Table mode state
  const [tableMode, setTableMode] = useState<{show: boolean; aitNum: string | null; action: ActionType | null}>({
    show: false,
    aitNum: null,
    action: null
  })
  ...
  const handleActionClick = useCallback((aitNum: string, action: ActionType) => {
    setTableMode({ show: true, aitNum, action })
  }, [])
    ...
    if (width > 0 && height > 0) {
      setNodes((currentNodes) => {
        ...
        for (let i = 0; i < newNodes.length; i++) {
          ...
          if ((node as any).parentId && sectionDimensions[(node as any).parentId as string]) {
            const parentDimensions = sectionDimensions[(node as any).parentId as string]
            const originalNode = initialNodes.find((n) => n.id === node.id)
            const originalParent = initialNodes.find((n) => n.id === (node as any).parentId)
  ...
  const nodesForFlow = useMemo(() => {
    return nodes.map((node) => {
      ...
      return { ...node as Node, data: nodeData }
    })
    ...
    <div className="h-full w-full relative">
      ...
      {tableMode.show ? (
        <SplunkAgGrid
          ...
          onBack={() => setTableMode({ show: false, aitNum: null, action: null })}
        />
      ) : (
        <>
          ...
          <div className="absolute top-16 right-4 z-10 max-w-md max-h-96 overflow-auto bg-white border rounded-lg shadow-lg p-4">
