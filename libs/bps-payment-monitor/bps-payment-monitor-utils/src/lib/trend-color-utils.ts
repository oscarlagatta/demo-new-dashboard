import type { SplunkDataItem } from "@bps-payment-monitor/data-access"
 ...
 */
function classifyStdVariation(stdVariation: number): "green" | "yellow" | "red" {
 ...
 */
function aggregateClassifications(classifications: ("green" | "yellow" | "red")[]): TrendColor {
 ...
 */
export function computeTrendColors(splunkData: SplunkDataItem[]): TrendColorMapping {
  const aitClassifications: { [aitNum: string]: ("green" | "yellow" | "red")[] } = {}
  ...
  Object.keys(aitClassifications).forEach((aitNum) => {
    trendColorMapping[aitNum] = aggregateClassifications(aitClassifications[aitNum])
  })
