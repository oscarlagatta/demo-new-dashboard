...
export const CustomNode = ({ data, id }: NodeProps<CustomNodeType>) => {
  ...
  const getCardClassName = () => {
    let baseClass = "border-2 border-[rgb(10,49,97)] shadow-md cursor-pointer transition-all duration-200 min-h-[24px]"
  ...
  return (
    <Card className={getCardClassName()} onClick={handleClick}>
      ...
      <CardContent className="p-2 pt-0">
        <div className="flex space-x-1 transition-all duration-200 min-h-[24px]">
          {inDefaultMode && (
            <>
              <LoadingButton
